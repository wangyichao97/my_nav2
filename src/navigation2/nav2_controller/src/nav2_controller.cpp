...
: LifecycleNode("controller_server", "", true),                              // 调用基类 LifecycleNode 的构造函数，创建名为 "controller_server" 的生命周期节点，启用了参数接口。
  progress_checker_loader_("nav2_core", "nav2_core::ProgressChecker"),      // 创建 progress_checker_loader_ 插件加载器，用于加载 ProgressChecker 类型的插件。第一个参数 "nav2_core" 表示插件所在的包，第二个参数表示要加载的接口基类，即 nav2_core::ProgressChecker。
  // 设置默认的进展检查器 ID 和类型。
...
ControllerServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  try {
    progress_checker_type_ = nav2_util::get_plugin_type_param(node, progress_checker_id_);      // 获取插件类型
  ...
  std::string & current_controller)
{
  ...
  {
    // 如果找不到指定的控制器且控制器列表中只有一个控制器，并且请求中没有明确指定控制器名称，则默认使用唯一加载的控制器
    ...
    // 将 current_controller 设置为找到的控制器名称
